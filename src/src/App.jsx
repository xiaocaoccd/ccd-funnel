      ...
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-center gap-4">
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-stone-200 text-stone-500 disabled:opacity-30 disabled:cursor-not-allowed hover:enabled:bg-amber-100 transition"
          >
            ←
          </button>

          <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-stone-800 tabular-nums">{activeYear} 年</span>
            {activeYear === CURRENT_YEAR && (
              <span className="text-xs bg-amber-400 text-white px-1.5 py-0.5 rounded-full">今年</span>
            )}
            {activeYear !== CURRENT_YEAR && (
              <button
        ...
        {n(row.daodian) > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-4">
            <h3 className="text-sm font-semibold text-stone-700 mb-3">🔻 {activeYear} 年 {MONTHS[activeMonth]} 用户转化漏斗</h3>
            <div className="space-y-2">
        ...
        {/* ── 图表 ── */}
        <Collapse title={`📈 ${activeYear} 年全年营收 · 毛利 · 成本趋势`}>
        ...
        <Collapse title={`👥 ${activeYear} 年全年到店 vs 成交趋势`}>
          <ResponsiveContainer width="100%" height={180}>
